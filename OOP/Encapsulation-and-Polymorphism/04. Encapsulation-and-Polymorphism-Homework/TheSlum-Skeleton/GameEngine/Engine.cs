...
namespace TheSlum.GameEngine
{
    ...
    public class Engine
    {
        ...
        protected virtual void CreateCharacter(string[] inputParams)
        {
            ...
            if (hero == "warrior")
            {
                Warrior war = new Warrior(id, x, y, team);
            ...
            {
                Healer heal = new Healer(id, x, y, team);
                this.characterList.Add(heal);
            }
        }

        protected void AddItem(string[] inputParams)
        {
            string heroId = inputParams[1];
            string itemType = inputParams[2];
            string itemId = inputParams[3];

            var hero = this.characterList.FirstOrDefault(x => x.Id == heroId);
            if (hero == null)
            {
                throw new ArgumentNullException("No such hero!");
            }

            if (itemType == "axe")
            {
                hero.AddToInventory(new Axe(itemId));
            }
            else if (itemType == "shield")
            {
                hero.AddToInventory(new Shield(itemId));
            }
            else if (itemType == "injection")
            {
                hero.AddToInventory(new Injection(itemId));
            }
            else if (itemType == "pill")
            {
                hero.AddToInventory(new Pill(itemId));
            }
        }
